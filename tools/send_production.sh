...
function print_help() {
    ...
    echo "  * GEANT4"
    echo "USAGE:"
    echo "  ./send_production.sh --run1 <run1> --run2 <run2> --serv <serv> --outsd <>"
    echo "  ./send_production.sh --rn <rn> --serv <serv> --outsd <out_sub_dir>"
    echo "  where:"
    ...
    echo "  <serv>        = select which machine to use: alice-serv<serv>"
    echo "                  it can be: 10, 12 or 14"
    echo "  <out_sub_dir> = subdirectory (it's a string!) within the output directory, for organizational purposes"
    echo "EXAMPLE:"
    echo "  ./send_production.sh --run1 1 --run2 100 --serv 10 --outsd 001"
}
...
function prepare_execution_script() {
    ...
    echo ""                                                                         >> ${run_file}
    ### merge csv files (note to self: can improve...)
...
for run in ${RUN_NUMBER_ARR[@]}; do
    ...
    RUN_DIR=${OUTPUT_DIR}/${OUTPUT_SUBDIR}/${STR_RUN}
    mkdir -p ${RUN_DIR}
